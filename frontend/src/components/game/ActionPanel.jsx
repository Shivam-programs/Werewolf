...
const copy = {
  ...
  Knight: ["Choose someone to protect", "knightProtect", "Grant protection"],
...
export function ActionPanel() {
  ...
  const castVote = () => {
    ...
    markVoteSubmitted();
    toast.success(`Your vote for ${selected} is sealed.`);
  };
  if (phase === "day")
    return (
  ...
  const submit = () => {
    ...
    if (event === "seerPeek") {
      socket.emit(event, { roomCode, target: selected });
      markActionSubmitted();
      return;
    }
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {aliveOthers.map((player) => (
          <button
            ...
            key={player.name}
            className={`rounded-xl border p-3 text-left text-sm transition ${selected === player.name ? "border-amber-300 bg-amber-300/10 text-amber-100" : "border-white/8 bg-white/2.5]text-zinc-300 hover:border-white/20"}`}
          >
